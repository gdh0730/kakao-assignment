...
import { SORTS, VIEWS } from "../constants";
import type { SortMode, ViewMode } from "../types";

type ViewSortControlsProps = {
...
export default function ViewSortControls({ currentView, currentSort, onViewChange, onSortChange }: ViewSortControlsProps) {
  ...
  return (
    <div className="mb-3 flex flex-col justify-between gap-3 sm:flex-row sm:items-center" aria-label="Todo 표시 방식">
      <div className="flex gap-1 border border-slate-200 bg-slate-50 p-1">
        {viewTabs.map((tab) => (
          <button
            key={tab.value}
            className={[
              "min-h-10 flex-1 px-4 font-bold transition sm:flex-none",
      ...
      <label className="flex items-center justify-between gap-2 text-sm font-bold text-slate-500" htmlFor="sortSelect">
        정렬
        <select className="min-h-10 border border-slate-200 bg-white px-3 text-ink outline-none focus:border-brand" id="sortSelect" value={currentSort} onChange={(event) => onSortChange(event.target.value as SortMode)}>
